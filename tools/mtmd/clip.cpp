...
struct clip_graph {
    ...
    ggml_cgraph * build_internvl() {
        // GLM-4V modification: class_embedding is optional
        const bool has_class_token = (model.class_embedding != nullptr);
        GGML_ASSERT(model.position_embeddings != nullptr);

        const int n_pos = has_class_token ? (n_patches + 1) : n_patches;
        ggml_tensor * inp = build_inp();
        
        // GLM-4V: transpose position_embeddings if needed
        // Expected: [n_embd, n_patches], actual might be [n_patches, n_embd]
        ggml_tensor * pos_embd = model.position_embeddings;
        if (pos_embd->ne[0] != n_embd && pos_embd->ne[1] == n_embd) {
            // Need to transpose
            pos_embd = ggml_cont(ctx0, ggml_transpose(ctx0, pos_embd));
        }

        // add CLS token only if it exists
        if (has_class_token) {
        ...
        ggml_tensor * cur = build_vit(
                                ...
                                norm_t,
                                hparams.ffn_op,
                                pos_embd,  // Use transposed version
                                nullptr);
        
        printf("DEBUG: after build_vit, cur shape: [%lld, %lld, %lld, %lld]\n", 
               cur->ne[0], cur->ne[1], cur->ne[2], cur->ne[3]);

        // remove CLS token only if it was added
        if (has_class_token) {
        ...
        printf("DEBUG: before pixel shuffle, cur shape: [%lld, %lld, %lld, %lld]\n", 
               cur->ne[0], cur->ne[1], cur->ne[2], cur->ne[3]);
        ...
        // pixel shuffle
        {
            ...
            printf("DEBUG: pixel shuffle - scale_factor=%d, height=%d, width=%d, n_embd=%d\n", 
                   scale_factor, height, width, n_embd);
            printf("DEBUG: trying to reshape [%lld, %lld] to [%d, %d, %d, %d]\n",
                   cur->ne[0], cur->ne[1], n_embd * scale_factor, height / scale_factor, width, bsz);
            cur = ggml_reshape_4d(ctx0, cur, n_embd * scale_factor, height / scale_factor, width, bsz);
            printf("DEBUG: after reshape_4d, cur shape: [%lld, %lld, %lld, %lld]\n",
                   cur->ne[0], cur->ne[1], cur->ne[2], cur->ne[3]);
            cur = ggml_permute(ctx0, cur, 0, 2, 1, 3);
            printf("DEBUG: after permute 1, cur shape: [%lld, %lld, %lld, %lld]\n",
                   cur->ne[0], cur->ne[1], cur->ne[2], cur->ne[3]);
            printf("DEBUG: trying cont_4d to [%d, %d, %d, %d]\n",
                   n_embd * scale_factor * scale_factor, height / scale_factor, width / scale_factor, bsz);
            cur = ggml_cont_4d(ctx0, cur,
                n_embd * scale_factor * scale_factor,
                height / scale_factor,
                width / scale_factor,
                bsz);
            printf("DEBUG: after cont_4d, cur shape: [%lld, %lld, %lld, %lld]\n",
                   cur->ne[0], cur->ne[1], cur->ne[2], cur->ne[3]);
            cur = ggml_permute(ctx0, cur, 0, 2, 1, 3);
            printf("DEBUG: after permute 2, cur shape: [%lld, %lld, %lld, %lld]\n",
                   cur->ne[0], cur->ne[1], cur->ne[2], cur->ne[3]);
            // flatten to 2D
            printf("DEBUG: flatten to 2D - target: [%d, %lld]\n",
                   n_embd * scale_factor * scale_factor, cur->ne[1] * cur->ne[2]);
            cur = ggml_cont_2d(ctx0, cur,
                n_embd * scale_factor * scale_factor,
                cur->ne[1] * cur->ne[2]);
            printf("DEBUG: after flatten, cur shape: [%lld, %lld]\n", cur->ne[0], cur->ne[1]);
            
            // GLM-4V: Reduce 6144 -> 4096 (select first 4096 channels)
            // This is a workaround since GLM-4V doesn't have a 6144->4096 weight
            const int target_dim = 4096;
            if (cur->ne[0] > target_dim) {
                printf("DEBUG: reducing from %lld to %d dimensions\n", cur->ne[0], target_dim);
                // Create a view that selects only the first 4096 dimensions
                cur = ggml_view_2d(ctx0, cur,
                    target_dim, cur->ne[1],
                    ggml_row_size(cur->type, cur->ne[0]), 0);
                printf("DEBUG: after dimension reduction, cur shape: [%lld, %lld]\n", cur->ne[0], cur->ne[1]);
            }
        }
        ...
        // projector (always using GELU activation)
        {
            ...
            printf("DEBUG: before build_norm, cur shape: [%lld, %lld]\n", cur->ne[0], cur->ne[1]);
            printf("DEBUG: mm_0_w shape: [%lld, %lld], mm_0_b shape: [%lld]\n",
                   model.mm_0_w->ne[0], model.mm_0_w->ne[1], model.mm_0_b->ne[0]);
            cur = build_norm(cur, model.mm_0_w, model.mm_0_b, NORM_TYPE_NORMAL, 1e-5, -1);
            printf("DEBUG: after build_norm\n");
            printf("DEBUG: mm_1_w shape: [%lld, %lld], mm_1_b shape: [%lld]\n",
                   model.mm_1_w->ne[0], model.mm_1_w->ne[1], model.mm_1_b->ne[0]);
            
            // GLM-4V: Use mm_2 as down projection if available, otherwise fall back to mm_3
            ggml_tensor * down_w = model.mm_2_w ? model.mm_2_w : model.mm_3_w;
            ggml_tensor * down_b = model.mm_2_b ? model.mm_2_b : model.mm_3_b;
            printf("DEBUG: down_w shape: [%lld, %lld], down_b shape: [%lld]\n",
                   down_w->ne[0], down_w->ne[1], down_b->ne[0]);
            ...
            cur = build_ffn(cur,
                ...
                FFN_GELU,
                -1);
            printf("DEBUG: after build_ffn\n");
            
            // GLM-4V: Apply final projection if mm_2 was used
            if (model.mm_2_w && model.mm_3_w) {
                printf("DEBUG: applying final projection with mm_3\n");
                cur = ggml_mul_mat(ctx0, model.mm_3_w, cur);
                if (model.mm_3_b) {
                    cur = ggml_add(ctx0, cur, model.mm_3_b);
                }
                printf("DEBUG: after final projection\n");
            }
        }
            ...
            case PROJECTOR_TYPE_INTERNVL:
                {
                    ...
                    // GLM-4V: also load layer 2 (down projection)
                    model.mm_2_w = get_tensor(string_format(TN_MVLM_PROJ_MLP, 2, "weight"), false);
                    model.mm_2_b = get_tensor(string_format(TN_MVLM_PROJ_MLP, 2, "bias"), false);
