...
class Glm4vVisionModel(MmprojModel):
    """GLM-4V Vision Model mmproj converter
    
    Supports AutoGLM-Phone-9B and GLM-4.1V-9B-Thinking models.
    Vision tensors are located in model.visual.*
    """
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert self.hparams_vision is not None
        # Adapt vision config parameter names to standard names
        self.hparams_vision["num_attention_heads"] = self.hparams_vision.get("num_heads", 12)
        self.hparams_vision["num_hidden_layers"] = self.hparams_vision.get("depth", 24)
    ...
    def set_gguf_parameters(self):
        ...
        # Set GLM-4V specific parameters  
        # Use INTERNVL as base since GLM-4V has similar architecture
        self.gguf_writer.add_clip_projector_type(gguf.VisionProjectorType.INTERNVL)
        self.gguf_writer.add_vision_attention_layernorm_eps(hparams.get("rms_norm_eps", 1e-05))
        
        # hidden_act - GLM-4V uses silu
        if hparams.get("hidden_act") == "silu":
            self.gguf_writer.add_vision_use_silu(True)
        elif hparams.get("hidden_act") == "gelu":
            self.gguf_writer.add_vision_use_gelu(True)
            
        # spatial merge size (GLM-4V specific feature for image compression)
        spatial_merge_size = hparams.get("spatial_merge_size", 2)
        if spatial_merge_size != 1:
            self.gguf_writer.add_vision_projector_scale_factor(spatial_merge_size)

    def tensor_force_quant(self, name, new_name, bid, n_dims):
        # Force F32 for position embeddings and other sensitive tensors
        if "position_embd" in new_name or "position_embedding" in new_name or "pos_embed" in new_name:
            return gguf.GGMLQuantizationType.F32
        return super().tensor_force_quant(name, new_name, bid, n_dims)
    
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        # GLM-4V linear layers have no bias, but llama.cpp expects them
        # Projector structure: layer 0 = norm (has bias), layer 1 = up, layer 2 = down, layer 3 = proj
        # Note: post_projection_norm already has bias, so we only generate for layers 1, 2, and 3
        import torch
        
        # Generate zero biases for projector MLPs (only for layers without bias)
        yield ("model.mm_projector.mlp.mlp.1.bias", torch.zeros(13696, dtype=torch.float32))
        yield ("model.mm_projector.mlp.mlp.2.bias", torch.zeros(4096, dtype=torch.float32))
        yield ("model.mm_projector.mlp.mlp.3.bias", torch.zeros(4096, dtype=torch.float32))
        
        # Note: GLM-4V doesn't use CLS token, so we don't generate v.class_embd
        # The C++ code has been modified to handle optional class_embedding

    def modify_tensors(self, data_torch, name: str, bid: int | None):
        del bid  # unused
        
        # Process extra generated tensors
        if name.startswith("model.mm_projector.mlp.mlp.") and name.endswith(".bias"):
            # Projector biases are already in the correct format
            return [(self.map_tensor_name(name), data_torch)]
        
        # Process visual tensors (model.visual.*)
        if name.startswith("model.visual."):
            # Remove model. prefix to get visual.* format
            name = name.replace("model.", "")
            
            # Process vision encoder and projection tensors
            # Include: blocks, patch_embed, embeddings, and merger (projection to text space)
            include_prefixes = ("visual.blocks.", "visual.patch_embed.", "visual.merger.", "visual.embeddings.")
            skip_prefixes = ("visual.downsample", "visual.post_conv_layernorm")
            
            if any(name.startswith(prefix) for prefix in skip_prefixes):
                return []  # Skip these specific tensors
            
            if not any(name.startswith(prefix) for prefix in include_prefixes):
                return []  # Skip other visual tensors
            
            # Map merger (projector) tensors
            # GLM-4V merger: post_projection_norm, up_proj, down_proj, proj (skip gate_proj)
            # Map to: layer 0 = norm, layer 1 = up, layer 2 = down, layer 3 = proj
            if "visual.merger." in name:
                # Skip gate_proj (not used)
                if "gate_proj" in name:
                    return []
                # Map to MLP structure
                name = name.replace("visual.merger.post_projection_norm", "model.mm_projector.mlp.mlp.0")
                name = name.replace("visual.merger.up_proj", "model.mm_projector.mlp.mlp.1")
                name = name.replace("visual.merger.down_proj", "model.mm_projector.mlp.mlp.2")
                name = name.replace("visual.merger.proj", "model.mm_projector.mlp.mlp.3")
            
            # Map position embedding - transpose to [hidden_size, num_patches]
            if "visual.embeddings.position_embedding" in name:
                name = name.replace("visual.embeddings.position_embedding", "v.position_embd")
                # GLM-4V original: [576, 1536] = [num_patches, hidden_size]
                # llama.cpp expects: [hidden_size, num_patches] = [1536, 576]
                # Transpose it - must materialize lazy tensor first
                import torch
                data_np = data_torch.numpy()  # Force materialization
                data_np = np.ascontiguousarray(data_np.T)  # Transpose [576,1536] -> [1536,576]
                data_torch = torch.from_numpy(data_np)
                logger.info(f"DEBUG: position_embd after transpose: {data_torch.shape}")
            
            # GLM-4V uses: visual.blocks.N.attn.{qkv,proj}, mlp.{gate_proj,up_proj,down_proj}
            # Mapping format already matches: visual.blocks.N.attn.*
            
            # Handle patch_embed.proj - GLM-4V uses Conv3D [C_out, C_in, T, H, W]
            # Split into two Conv2D layers for temporal dimension
            if "patch_embed.proj.weight" in name:
                if data_torch.ndim == 5:  # Conv3D: [C_out, C_in, T, H, W]
                    c_out, c_in, kt, kh, kw = data_torch.shape
                    assert kt == 2, f"Temporal kernel size is {kt}, expected 2"
                    # Split into two temporal frames
                    return [
                        (self.map_tensor_name(name), data_torch[:, :, 0, ...]),  # First frame
                        (self.map_tensor_name(name) + ".1", data_torch[:, :, 1, ...]),  # Second frame
                    ]
            
            # Handle QKV split - GLM-4V uses fused QKV attention
            if ".attn.qkv." in name:
                if data_torch.ndim == 2:  # weight
                    c3, _ = data_torch.shape
                else:  # bias
                    c3 = data_torch.shape[0]
                assert c3 % 3 == 0, f"QKV tensor size {c3} is not divisible by 3"
                c = c3 // 3
                wq = data_torch[:c]
                wk = data_torch[c: c * 2]
                wv = data_torch[c * 2:]
                return [
                    (self.map_tensor_name(name.replace("attn.qkv", "attn.q")), wq),
                    (self.map_tensor_name(name.replace("attn.qkv", "attn.k")), wk),
                    (self.map_tensor_name(name.replace("attn.qkv", "attn.v")), wv),
                ]
            
            return [(self.map_tensor_name(name), data_torch)]
        
        # Skip language model tensors - we only want vision parts for mmproj
        return []
